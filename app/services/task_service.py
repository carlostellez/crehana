...
class TaskService:
    ...
    def __init__(self):
        """
        Initialize the TaskService with sample data.
        """
    ...
    def get_task_by_id(self, task_id: int) -> Optional[Task]:
        ...
        """
        task_dict = next((task for task in self.tasks_data if task["id"] == task_id), None)
        if task_dict:
            return Task(
                id=task_dict["id"],
    ...
    def delete_task(self, task_id: int) -> bool:
        ...
        """
        task_index = next((i for i, task in enumerate(self.tasks_data) if task["id"] == task_id), None)
